...
class Distance( object ):
  ...
  def fitness_fake( self ):
    ...
    """
    return 0.0, np.array( [0] * self.desc_size )
...
class Distance_Angle( object ):
  """
  Class to calculate fitness based on distance and angle.
  """

  def __init__( self, exp_folder, log=None ):
    self.exp_folder = exp_folder
    self.log_name = log
    self.desc_size = 3 #size of descriptor for this fitness fnc()
  ...
  def fitness( self, sim_run ):
    ...
    @output: fitness (float), descriptor (np array)
    Calculate fitness based on vector (final - initial) norms, return descriptor
    based on distance traveled in x,y coordinates and the angle.
    This works for a single robot only.
    """
    ...
    initial, final = self.parser( sim_run )
    angle = get_angle( sim_run, self.exp_folder )
    ...
    return avg, np.array( [ np.average( vectors[:,:1] ), np.average( vectors[:,1:2] ), angle ] )
